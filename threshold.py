...
inputNiiImage, ImageSize, affine, header = utils.load_nii('input.nii')
...
# core calculation
#
ort_session = onnxruntime.InferenceSession("model_FLAIR_gdice.onnx")
input_image = utils.preprocess(inputNiiImage)
num_img = len(input_image)
# Process the input images slice by slice
SegResult = np.zeros_like(input_image)
completed = 0
for idx, img in enumerate(input_image):
    ...
    res = utils.postprocess_numpy(ort_outs[0])
    # pred = pred_tool.predict(img)
    completed += 100/ num_img
    SegResult[idx] = cv2.resize(res, ImageSize, interpolation=cv2.INTER_NEAREST)
statusMessage = "Progress:{0}%".format(completed)
